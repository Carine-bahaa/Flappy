...
struct Bird
{
    ...
    if (diff == 2)
    {
        ...
        if (AA.getElapsedTime().asSeconds() <= 2)
        {
            for (int i = 0; i < 5; i++)
            {
                if (i % 2 == 0)
                {
                    Pipes.PiUp[i].move(0, m);
                    Pipes.PiDown[i].move(0, m);
                }
                else
                {
                    Pipes.PiUp[i].move(0, -m);
            ...
            if ((int)AA.getElapsedTime().asSeconds() == 2)
            {
                m *= -1;
                AA.restart();
            }
